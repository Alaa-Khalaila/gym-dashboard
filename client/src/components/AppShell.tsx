...
export default function AppShell() {
  ...
  const sidebar = (
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center gap-2.5 px-4 py-5 border-b border-zinc-800">
        ...
        <span className="font-bold text-white text-base">GymDash</span>
      </div>
      ...
      <div className="px-3 py-4 border-t border-zinc-800 space-y-1">
        <div className="px-3 py-2 text-xs text-zinc-500 truncate">{user?.name}</div>
        <button
          onClick={handleLogout}
          className="flex items-center gap-3 w-full px-3 py-2.5 rounded-lg text-sm font-medium text-zinc-400 hover:bg-zinc-800 hover:text-white transition-colors"
        >
  ...
  return (
    <div className="flex h-screen bg-zinc-950 overflow-hidden">
      {/* Desktop sidebar */}
      <aside className="hidden lg:flex flex-col w-60 shrink-0 bg-zinc-900 border-e border-zinc-800">
        {sidebar}
      </aside>
      ...
      {sidebarOpen && (
        <div className="lg:hidden fixed inset-0 z-40 flex">
          ...
          <aside className="relative z-50 flex flex-col w-60 bg-zinc-900 border-e border-zinc-800">
            {sidebar}
          </aside>
        </div>
      )}
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <header className="lg:hidden flex items-center justify-between px-4 py-3 bg-zinc-900 border-b border-zinc-800">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="font-bold text-white text-sm">GymDash</span>
          </div>
          <LanguageToggle />
        </header>

        {/* Desktop top bar */}
        <header className="hidden lg:flex items-center justify-end px-6 py-3 bg-zinc-900 border-b border-zinc-800">
          <LanguageToggle />
        </header>
